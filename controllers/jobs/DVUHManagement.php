...
class DVUHManagement extends JQW_Controller
{
	 ...
	 */
	public function requestVbpk()
	{
		$jobType = 'DVUHRequestVbpk';
		$this->logInfo('DVUHRequestVbpk job start');
		...
		else
		{
			...
			{
				if (!isset($persobj->person_id))
					$this->logError("Fehler bei Vbpkabfrage, ungültige Parameter übergeben");
				else
				{
					$person_id = $persobj->person_id;

					$requestVbpkResult = $this->dvuhmasterdatamanagementlib->getVbpk($person_id);

					if (isError($requestVbpkResult))
					{
						$this->_logDVUHError(
							"Fehler bei Vbpkabfrage, person Id $person_id",
							$requestVbpkResult,
							$person_id
						);
					}
					elseif (hasData($requestVbpkResult))
					{
						$requestEkzArr = getData($requestVbpkResult);
		...
		$this->logInfo('DVUHRequestVbpk job stop');
	}
