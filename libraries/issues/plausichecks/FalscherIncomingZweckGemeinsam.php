 ...
 *
 */
class FalscherIncomingZweckGemeinsam extends PlausiChecker
{
	...
	private function _getFalscherIncomingZweck(
		...
		$status_clause = '';
		if (isset($status_clause) && !isEmptyArray($status_clause))
		{
		...
		$qry = "
			SELECT * FROM (
				SELECT
					...
					bisio.bisio_id, COUNT(DISTINCT zw.zweck_code) AS anzahl
				FROM
				...
				WHERE
					zw.zweck_code IN ('1', '2', '3')
					AND stg.melderelevant
					AND pre.bismelden
					AND bisio.von::date < sem.ende AND bisio.bis::date > sem.start
					{$studiensemester_clause}
					{$status_clause}
				GROUP BY bisio.bisio_id, prestudent_id, stg.oe_kurzbz, stg.studiengang_kz
			) zwecke
			WHERE anzahl > 2";
