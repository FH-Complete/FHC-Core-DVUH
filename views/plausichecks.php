...
$this->load->view(
	...
	array(
		...
		'dialoglib' => true,
		'customCSSs' => array('public/css/sbadmin2/admintemplate.css'),
		'customJSs' => array('public/extensions/FHC-Core-DVUH/js/plausichecks.js')
	)
);
?>
			...
			<div class="row">
				...
				<div class="col-xs-3 col-lg-4 form-group">
					...
					<select class="form-control" name="fehler_kurzbz" id="fehler_kurzbz">
						<option value="" selected="selected">Alle</option>';
						<?php foreach ($fehlerKurzbzCodeMappings as $fehler_kurzbz => $fehler):?>
							<option value="<?php echo $fehler_kurzbz ?>">
								<?php echo $fehler_kurzbz.' ('.$fehler['fehlercode'].')' ?>
							</option>
						<?php endforeach; ?>
					</select>
				</div>
